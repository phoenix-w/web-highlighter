(() => {
  ...
  function allColorValues() {
    return allColors().map((c) => ({
      name: c.name,
      bg: colorWithOpacity(c.hex, settings.opacity),
    }));
  }
  ...
  function colorValueByIndex(index) {
    ...
    return colorWithOpacity(colors[index].hex, settings.opacity);
  }
  ...
  // --- Range serialization via XPath + offset ---

  function getXPath(node) {
    if (node.nodeType === Node.TEXT_NODE) {
      return getXPath(node.parentNode) + "/text()[" + textNodeIndex(node) + "]";
    }
  ...
  function wrapTextNode(textNode, range, bg, id) {
    ...
    mark.addEventListener("click", (e) => {
      e.stopPropagation();
      showToolbar(e.clientX, e.clientY, mark);
    });

    return mark;
  }

  function applyHighlight(range, bg, id) {
  ...
  function removeHighlight(mark) {
    ...
    const allMarks = document.querySelectorAll(
      `mark.web-highlighter[data-highlight-id="${id}"]`
    );
    allMarks.forEach((m) => {
    ...
    chrome.storage.local.get(pageKey(), (result) => {
      ...
      chrome.storage.local.set({ [pageKey()]: updated });
    });
  }
  ...
  function showToolbar(x, y, existingMark) {
    ...
    toolbar.style.top = Math.max(y - 44, 4) + "px";
  ...
  function highlightSelection(color) {
    ...
    const bg = color || colorValueByIndex(0);

    applyHighlight(range, bg, id);
  ...
  function matchesShortcut(e, combo) {
    ...
    const keyMatch = eventKey === key || (codeKey && codeKey === key);
    return (
      keyMatch &&
      e.ctrlKey === needCtrl &&
      e.altKey === needAlt &&
  ...
  chrome.runtime.onMessage.addListener((msg) => {
    ...
    if (msg.action === "remove-highlight") {
      const mark = document.querySelector(
        `mark.web-highlighter[data-highlight-id="${msg.id}"]`
      );
      if (mark) removeHighlight(mark);
    }
    if (msg.action === "get-highlights") {
      chrome.storage.local.get(pageKey(), (result) => {
        chrome.runtime.sendMessage({
          action: "highlights-data",
